...
def cons1(a):  # 列写第一种约束
    return {'type': 'ineq', 'fun': lambda du: du[a] + 2}

def cons2(a): #列写第二种约束
    return {'type': 'ineq', 'fun': lambda du: 2-du[a]}
...
def objective(du,x,last_u,trajs):
    y = np.zeros((1,len(du)))
    u0 = last_u
    for i in range(len(du)):
        x,u0 = predict_model(x, u0,du[i])
        y[:,i] = x
    # print(f'predict_horizon:{y}')
    J = cost_function(du,y, trajs,last_u)  #, alpha, beta
    return J
...
if __name__ == '__main__':
    ...
    solution = minimize(objective, du, method='SLSQP', args=(x,last_u,trajs), options={'eps': 1e-08},constraints=con)
    t1 = time.time()
